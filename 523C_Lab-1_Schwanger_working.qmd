...
```
#this gauge didn't start recording until 1985-10-01
...
little_co_gaps
```
#this site has a gap of 731 days starting ~1993-01-01 and ending 1994-12-31
...
co_r_gaps
```
#Colorado R Below Hoover Dam site had a gap of 239 days ending in 2023-03-26
 
```{r}
attr(raw, "variableInfo")
```
#confirmed Flow is in cfs
...
```
# A left join was used to preserve all flow records in the raw dataset, retaining rows even where no matching site metadata existed. 
...
```{r}
joined[joined$site_no == "09085000", ]
```
...
ft_summary_table <- flextable(summary_table) |>
  set_header_labels( period = "Period", avg_days_below = "Average Days per Year Below\nThreshold at Lees Ferry") |>
  autofit() |>
...
left_map_panel <- ggplot() +
  ...
  annotate("text", x = -111.5, y = 37.5, label = "Utah", color = "gray30", size = 4) +
  annotate("text", x = -111.5, y = 35.5, label = "Arizona", color = "gray30", size = 4)
...
#hmm, are data gaps messing things up again? 

```{r}
flow_weighted_gaps_removed <- flow_weighted |>
...
left_map_panel_2 <- ggplot() +
  geom_polygon(data = ut_az_states, aes(x = long, y = lat, group = group),
               fill = "gray90", color = "white") +
  geom_path(data = flow_weighted_gaps_removed, aes(x = weighted_lon, y = weighted_lat, color = water_year)) +
  geom_point(data = flow_weighted_gaps_removed, aes(x = weighted_lon, y = weighted_lat, color = water_year)) +
  scale_color_viridis_c() +
  coord_fixed(1.3, xlim = c(-115, -109), ylim = c(35, 38)) +
  labs(title = "Flow-Weighted Center of Basin", x = NULL, y = NULL, color = "Water Year") +
  annotate("text", x = -111.5, y = 37.5, label = "Utah", color = "gray30", size = 4) +
...
```{r}
right_map_panel_2 <- ggplot(flow_weighted_gaps_removed, aes(x = water_year, y = weighted_lat)) +
  geom_line() +
  geom_smooth(method = "lm", color = "black") +
  labs(title = "Weighted Mean Latitude Over Time", x = "Water Year", y = "Weighted Mean Latitude") +
  annotate("text", x = 1993, y = 37.14, label = "1993", size = 3) 
...
```{r}
left_map_panel_2 + right_map_panel_2
```
#so are the spikes 1993 and 1995? 
...
pred_v_actual_plot <- aug |>
  ...
  geom_text(data = filter(aug, abs(.resid) > 0.2), aes(label = water_year), size = 3, nudge_y = 0.06)
...
aug |> 
  arrange(desc(.resid)) |> 
  select(water_year, .resid) |> head(5)
```
...
> ...The most notable outlier is 1983, which the model substantially underpredicted. This was an exceptionally wet year driven by record Rocky Mountain snowpack combined with a strong El Niño, producing flows far outside the range the model was trained on CITATION. The residuals are approximately normally distributed and centered near zero, though the long right tail driven by 1983 suggests the model struggles with extreme wet years. An additional predictor that would improve the model is spring snowpack levels at upstream SNOTEL stations, which directly drives spring runoff volume and would help the model anticipate extremely wet years like 1983. The Palmer Drought Severity Index or similar soil moisture measure would also improve the model, as it would capture how much upstream precipitation is absorbed by dry soils before reaching the channel.
