...
```{r}
voronoi_crop <- voronoi |>
  st_intersection(voronoi, conus_boundary)

plot(voronoi_crop)
```

```{r}
#ugh got warning and the plot is wrong
#need to only pass geometry, then add it back

voronoi_crop <- st_intersection(st_geometry(voronoi), conus_boundary) |>
  st_as_sf() |>
  mutate(id = 1:n())
...
```{r}
triangulated_crop <- st_intersection(st_geometry(triangulated), conus_boundary) |>
  st_as_sf() |>
  mutate(id = 1:n())

plot(triangulated_crop)
```

```{r}
#gah ok, apparently don't need to drop geometry for st_filter
gridded_crop <- st_filter(gridded, conus_boundary)
...
```{r}
hexagonal_crop <- st_filter(hexagonal, conus_boundary)
...
st_is_valid(conus_boundary_simple)

# Check: Does it still contain all of CONUS?
st_bbox(conus_boundary_simple)  # Should cover roughly [-125, 24] to [-66, 49]
#hmm numbers don't match up
st_bbox(conus_boundary)
#but they are similar to the original, so probably ok

# Check: Visual comparison
plot(conus_boundary, main = "Original")
plot(conus_boundary_simple, main = "Simplified")
```
:::
...
> The counties tessellation standard deviation is large, meaning some counties are significantly bigger than others. For point in polygon comparisons, this could mean…
The voronoi tessellation is similar to counties, with the same number of features and mean area, but a slightly smaller standard deviation in area. This means…
For both the hexagonal and square tessellations, the standard deviation is 0. This makes sense as the tessellations are all the same size, but it means they’re not taking into account any geographical geometries and are completely arbitrary. For point in polygon comparisons this means…
The triangulated tessellation has almost twice the amount of features than any other tessellation, which could require significantly more computational bandwidth for analysis. 
