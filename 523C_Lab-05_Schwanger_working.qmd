...
```{r}
dists <- as.matrix(dist(camels |> select(gauge_lon, gauge_lat)))
...
> The best hyperparameter combination was mtry = 20 and min_n = 3, which resulted in a rsq of .881.
The model appears sensitive to the mtry parameter as there's a clear pattern in the plot. min_n shows slightly less sensitivity. 
...
> The top predictors by a large margin are p_mean and aridity. Both are climate drivers of streamflow and reflect a strong physical mechanism. 
